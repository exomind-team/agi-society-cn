...
新账号验证运行：`35312128829`，提交 `57511c6`。

- Runner → 腾讯云 HTTP 探测：HTTP 200；TCP 建连 0.260 s；总计 0.780 s；
- SCP 归档大小：17,621,881 B；开始 `13:47:21`，结束 `13:52:39`；耗时 318.277 s；
- 实测吞吐：55,366.5 B/s，约 54.1 KiB/s；
- 受限 SSH 密钥、强制命令、SCP 上传和远端原子激活全部成功。

前一轮的粗略估算约为 14.7 KB/s，说明该跨境上传速率存在明显波动；连接建立本身稳定，瓶颈主要在持续传输阶段。
